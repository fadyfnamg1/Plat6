...
import { driftPayouts } from '../lib/payoutEngine';
...
export default function TradingPlatform() {
  ...
  // ── Dynamic payout drift ──────────────────────────────────────────────
  // Every ~18s, nudge each pair's payout and re-rank the list (highest
  // payout first). Keeps the currently-selected pair's payout in sync too.
  useEffect(() => {
    const iv = setInterval(() => {
      const { markets: cur, setMarkets, currentMarket: curSel, setCurrentMarket } = useStore.getState();
      if (!cur.length) return;
      const updated = driftPayouts(cur);
      setMarkets(updated);
      if (curSel) {
        const matched = updated.find(m => m.id === curSel.id);
        if (matched && matched.payout !== curSel.payout) setCurrentMarket({ ...curSel, payout: matched.payout });
      }
    }, 18000);
