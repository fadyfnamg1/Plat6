...
export default function TradingChart() {
  ...
  // tradeWsRef subscribes to that same @trade stream and a rAF loop applies
  // every tick straight to the forming candle, so both move in lockstep.
  ...
  const buildChart = useCallback(() => {
    ...
    if (tradeWsRef.current) { tradeWsRef.current.close(); tradeWsRef.current = null; }
    ...
    function open() {
      ...
      ws.onmessage = (e) => {
        ...
        try {
          ...
          if (!isNaN(p)) latestTickRef.current = p;
        } catch {}
      };
      ws.onclose = () => { if (tradeWsRef.current === ws && buildId === buildIdRef.current) setTimeout(open, 1500); };
